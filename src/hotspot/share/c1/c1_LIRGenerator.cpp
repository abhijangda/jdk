...
LIR_Opr LIRGenerator::new_instance(LIR_Opr dst, ciInstanceKlass* klass, bool is_unresolved, LIR_Opr scratch1, LIR_Opr scratch2, LIR_Opr scratch3, LIR_Opr scratch4, LIR_Opr klass_reg, CodeEmitInfo* info) {
  ...
  if (UseFastNewInstance && klass->is_loaded()
    ...
    return LIR_OprFact::longConst((uint64_t)instance_size);
  } else {
    CodeStub* slow_path = new NewInstanceStub(klass_reg, dst, klass, info, Runtime1::new_instance_id);
    __ branch(lir_cond_always, slow_path);
    __ branch_destination(slow_path->continuation());
    //Do not add append_heap_event here because slow path will go to InstanceKlass.allocate_instance.
...
void LIRGenerator::append_heap_event(Universe::HeapEventType event_type, LIR_Opr dst_or_new_obj, LIR_Opr src_or_obj_size) {
  ...
  //Called for FieldSet for cas_obj 
  ...
  if (true) {
    LIR_Address* heap_event_counter_addr = new LIR_Address(getThreadPointer(), JavaThread::heap_events_offset(), T_LONG);
    ...
    LIR_Address* heap_events_addr_src = new LIR_Address(getThreadPointer(), counter, JavaThread::heap_events_offset(), T_LONG); //Object size is in Integer and not Long
    LIR_Address* heap_events_addr_dst = new LIR_Address(getThreadPointer(), counter, JavaThread::heap_events_offset() + 8, T_LONG);
    ...
    if (event_type == Universe::HeapEventType::FieldSet) {
      ...
      if (src_or_obj_size.is_constant() && src_or_obj_size.as_jobject() != NULL) {
        __ move(src_or_obj_size, tmp2);
      } else if (src_or_obj_size.is_constant()) {
        __ move(LIR_OprFact::longConst(0), tmp2);
      } else {
        __ move(src_or_obj_size, tmp2);
      }
      __ store(tmp2, heap_events_addr_src);

      LIR_Address* orig_addr = dst_or_new_obj->as_address_ptr();      
    ...
    } else if (event_type == Universe::HeapEventType::NewObject || event_type == Universe::HeapEventType::NewArray) {   
      ShouldNotReachHere();
      //Not updated
      // if (src_or_obj_size.is_constant()) {
      //   LIR_Const* lirconst =  src_or_obj_size.pointer()->as_constant();
      //   uint64_t src_const = lirconst->as_jlong();
    ...
    __ transfer_events(counter, LIR_OprFact::longConst(MaxHeapEvents*sizeof(Universe::HeapEvent)));
  }
...
void LIRGenerator::append_copy_array(LIR_Opr dst_array, LIR_Opr src_array, LIR_Opr dst_offset, LIR_Opr src_offset, LIR_Opr count) {
  ...
  if (true) {
    ...
    __ shift_left(heap_events_idx, exact_log2_long(sizeof(Universe::HeapEvent)), heap_events_idx);

    LIR_Address* heap_events_addr_src = new LIR_Address(getThreadPointer(), heap_events_idx, JavaThread::heap_events_offset(), T_LONG);
    LIR_Address* heap_events_addr_dst = new LIR_Address(getThreadPointer(), heap_events_idx, JavaThread::heap_events_offset() + 8, T_LONG);
    ...
    __ logical_or(tmp, LIR_OprFact::longConst(Universe::HeapEventType::CopyArray), tmp);
    __ store(tmp, heap_events_addr_src);
    ...
    // __ store(LIR_OprFact::longConst(Universe::HeapEventType::CopyArrayOffsets), heap_events_addr_type);
    // __ store(src_offset, heap_events_addr_src);
    // __ store(dst_offset, heap_events_addr_dst);

    heap_events_addr_src = new LIR_Address(getThreadPointer(), heap_events_idx, JavaThread::heap_events_offset() + 16, T_LONG);
    // uint64_t encoded_event_copy_array_length = Universe::encode_heap_event_src(Universe::HeapEventType::CopyArrayLength, 0L);
