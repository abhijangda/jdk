...
//   // if ((Universe::heap_event_counter & ((1<<Universe::LOG_MAX_EVENT_COUNTER) - 1)) == 0 && Universe::heap_event_counter > 1) 

//   static int i = 0; //Universe::heap_event_counter/(1<<Universe::LOG_MAX_EVENT_COUNTER);
//   Universe::heap_event_counter = 1 << Universe::LOG_MAX_EVENT_COUNTER;
//   // AllObjects all_objects;
//   // Universe::heap()->object_iterate(&all_objects);
//   printf("checking %d\n", i++);

//   for (uint64_t j = 0; j < Universe::heap_event_counter; j++) {
//     // heap_events[j].address.src, heap_events[j].address.dst;
//   }
...
void MacroAssembler::append_two_heap_events(Address events, Universe::HeapEventType event_type[2], Register dst[2], Register src[2], 
                                            Register cntr_reg) {
  ...
  events = Address(r15_thread, cntr_reg, Address::times_1, (int)JavaThread::heap_events_offset());
  movq(events, src[0]);

  events = Address(r15_thread, cntr_reg, Address::times_1, JavaThread::heap_events_offset() + 8);
  shlq(dst[0], 15);
  if (event_type[0] != 0) {
    uint64_t encoded_dst = Universe::encode_heap_event_dst(event_type[0], 0);
    orq(dst[0], (int32_t)encoded_dst);
  }
  movq(events, dst[0]);
  shrq(dst[0], 15);
...
void MacroAssembler::append_two_heap_events(Address events, Universe::HeapEventType event_types[2], 
  ...
  for (int n = 0; n < NUM_EVENTS; n++) {
    if (srcs[n].is_register()) {
      pinsrq(tmp_vec, srcs[n].as_register(), n);
    } else {
      assert(srcs[n].is_constant(), "sanity");
      if (srcs[n].as_constant() == 0) {
        pxor(tmp_vec, tmp_vec);
...
void MacroAssembler::append_newarray_event(Universe::HeapEventType arraytype, Register obj, RegisterOrConstant size, 
                                         Register tmp1, bool preserve_tmp1, 
                                         Register tmp2, bool preserve_tmp2,
                                         bool preserve_flags) {
