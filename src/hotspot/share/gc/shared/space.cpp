    ...
    if (cast_to_oop(cur_obj)->is_gc_marked()) {
      ...
      if (cur_obj == compact_top && dead_spacer.insert_deadspace(cur_obj, end)) {
        ...
        if (InstrumentHeapEvents && cur_obj != end) {
          Universe::add_heap_event(Universe::HeapEventType::ClearContiguousSpace, Universe::HeapEvent{(uint64_t)cur_obj, (uint64_t)end});
        }
  ...
  while (cur_obj < end_of_live) {
    if (!cast_to_oop(cur_obj)->is_forwarded()) {
      ...
      if (InstrumentHeapEvents)
        Universe::add_heap_event(Universe::HeapEventType::MoveObject, Universe::HeapEvent{(uint64_t)(void*)cast_to_oop(cur_obj), (uint64_t)(void*)cast_to_oop(cur_obj)->forwardee()});
